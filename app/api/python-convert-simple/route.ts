...
export async function POST(request: NextRequest) {
  try {
    ...
    const labTitle = lab_info?.labTitle || "Notebook Forge .ipynb to Word converter"
    ...
    // Helper function to generate content with working page numbers
    const generateFieldChildren = (field: any, filename: string, labTitle: string): TextRun[] => {
          ...
          switch (pageFormat) {
            case "brackets":
              return [
                ...
                new TextRun({ text: "1", font: "Calibri", size: 22 }),
                new TextRun({ text: "]", font: "Calibri", size: 22 })
              ]
            case "page_word":
              return [
            ...
            case "roman":
              return [new TextRun({ text: "i", font: "Calibri", size: 22 })]
            case "plain":
              return [new TextRun({ text: "1", font: "Calibri", size: 22 })]
            default:
              return [new TextRun({ text: "1", font: "Calibri", size: 22 })]
          }
    ...
    // Create 3-column header with proper page number fields
        ...
        new TableRow({
          children: [
            new TableCell({
              children: [
                new Paragraph({
                  children: generateFieldChildren(lab_info?.headerLeft, filename, labTitle),
            ...
            new TableCell({
              children: [
                new Paragraph({
                  children: generateFieldChildren(lab_info?.headerCenter, filename, labTitle),
                  alignment: AlignmentType.CENTER,
                }),
              ],
              width: { size: 33.33, type: WidthType.PERCENTAGE },
            ...
            new TableCell({
              children: [
                new Paragraph({
                  children: generateFieldChildren(lab_info?.headerRight, filename, labTitle),
    ...
    // Create 3-column footer with watermark in the center and proper page number fields
        ...
        new TableRow({
          children: [
            new TableCell({
              children: [
                new Paragraph({
                  children: generateFieldChildren(lab_info?.footerLeft, filename, labTitle),
                  alignment: AlignmentType.LEFT,
                }),
              ],
              width: { size: 33.33, type: WidthType.PERCENTAGE },
            ...
            new TableCell({
              children: [
                new Paragraph({
                  children: generateFieldChildren(lab_info?.footerCenter, filename, labTitle),
                  alignment: AlignmentType.CENTER,
                }),
                // Add watermark with link to NotebookForge
                new Paragraph({
                  children: [
                    new TextRun({
                      text: "Made with ",
                    ...
                    new ExternalHyperlink({
                      ...
                      link: "https://v0-jupyter-to-word-converter.vercel.app/",
                    }),
                  ],
                  alignment: AlignmentType.CENTER,
                  spacing: { before: 50 },
            ...
            new TableCell({
              children: [
                new Paragraph({
                  children: generateFieldChildren(lab_info?.footerRight, filename, labTitle),
                  alignment: AlignmentType.RIGHT,
                }),
              ],
              width: { size: 33.33, type: WidthType.PERCENTAGE },
    ...
    // Create document with proper page numbering support
...
function processMarkdownCell(cell: any, sections: any[]) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Enhanced heading detection - now handles ####, #####, ######
